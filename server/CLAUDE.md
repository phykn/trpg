...
### Dev (`APP_ENV=dev`, default) — local filesystem
...
### Release (`APP_ENV=release`) — Supabase Postgres + Storage
...
- **Saves → Postgres** via PostgREST. Five tables, all keyed on `game_id`:
  - `games(game_id PK, meta jsonb, updated_at)` — meta.json mirrored as a single jsonb column. No column decomposition (saves migrations when meta evolves).
  - `entities(game_id, kind, id, data jsonb, updated_at)` PK `(game_id, kind, id)` — every per-entity JSON file maps to one row. Indexed on `(game_id, kind)` for "load all characters of game X." `kind ∈ {characters, items, locations, races, skills, quests, chapters, campaigns}`.
  - `log_entries(game_id, log_id int, entry jsonb)` PK `(game_id, log_id)` — `log_id = entry.id` (monotonic, app-managed). Client dedupes by id.
  - `history_entries(game_id, seq bigserial, entry jsonb)` PK `(game_id, seq)` — `RULES.memory.turn_log_size` tail loaded.
  - `dialogue_entries(game_id, seq bigserial, entry jsonb)` PK `(game_id, seq)` — `RULES.memory.recent_dialogue_turns` tail loaded.
...
- **Scenarios → Storage** bucket. Layout mirrors `../scenarios/<profile>/...` 1:1 — `<bucket>/<profile>/{world.md, start.json, player_template.json, profile.json, races/*.json, characters/*.json, items/*.json, ...}`. Author scenarios locally on dev fs, then upload the dir tree to the bucket.
