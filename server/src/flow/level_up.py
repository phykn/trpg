...
async def run_level_up(
    ...
    *,
    ...
    if skill_id is not None and skill_id in state.skills:
        ...
        if skill_id not in actor.learned_skill_ids:  # ssot-allow: write path — graph rebuilds at next turn boundary.
            actor.learned_skill_ids.append(skill_id)  # ssot-allow: write path — graph rebuilds at next turn boundary.
