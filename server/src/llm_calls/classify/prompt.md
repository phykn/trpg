...
| 4 | rest | `{"action":"rest"}` | Long sleep/camp. Not in combat. **Trigger verbs (any of)**: 잔다 / 잠을 청한다 / 잠자리에 든다 / 푹 쉰다 / 휴식한다 / 자리를 잡고 (눕는다·앉아 잔다·호흡한다 + 회복 의도) / 캠프를 친다 / 모닥불을 피우고 쉰다 / 야영한다. **체력 회복**·**HP 회복**·**충분히 쉰다** 같은 명시적 회복 의도가 있으면 hedging("잠시·잠깐") 단어와 함께 와도 `rest`. 짧은 호흡(단순 "한숨 돌린다", 회복 의도 없음) → `pass`. |
...
| 11.5 | give | `{"action":"give","from_id":"<src>","to_id":"<dst>","item_id":"<id>"}` | Free transfer (gift / lend / hand-over / corpse loot / accept). Player input must name *which item* moves (must hit `entities`·`equipment`·`corpses[*].inventory[*].id`). Direction: NPC→Player receive/borrow/take (`from=npc_id, to=player_01`); Player→NPC give/hand-over/pass (`from=player_01, to=npc_id`); corpse loot (`from=corpse_id, to=player_01` — corpse comes from `surroundings.corpses[*].id`). **Two-way swap** (give and receive at once — "이거 줄테니 그거 줘", "이걸 너의 X와 바꾸자", "교환하자/맞바꾸자/주고받자"): use `chain` to bundle two gives so both fire — one player→npc with `item_id` from player's `inventory`, one npc→player with `item_id` from `entities[npc].carryables[*].id`. Emitting only one direction strands one item forever. If either side's item is missing or ambiguous → `pass`. Refuse/avoid/deflect → `pass`. Negotiation/persuasion/begging to be lent → `roll`(CHA) — the essence is winning consent, not the transfer itself. |
...
| 14 | chain | `{"action":"chain","parts":[<sub-action>, <sub-action>, ...]}` | Compound input carrying **engine action + a separate intent** ("약초 먹고 검을 든다" = use+equip, "검 들고 광장 상인에게 다가간다" = equip+pass, "**검을 꺼내 경계하며 전진한다**" = equip+pass, "약초 마시고 광장으로 간다" = use+move, "**단검을 뽑아 공격한다**" = equip+combat, "**광장으로 가서 상인을 친다**" = move+combat). 2-4 parts. Non-tail parts are limited to `use`/`equip`/`unequip`/`buy`/`sell`/`give`/`move`/`pass`. The **tail** part may additionally be `combat` (engine runs prefix parts, then transitions to the combat phase). `rest`·`flee`·`roll`·`reject`·`summon_combat`·`recruit`·`dismiss` remain forbidden anywhere in chain — phase conflict / no compose. A single-branch compound ("뒤져서 연다" = single roll) is **not** chain — emit as a single action. **Wrap a second verb that's an unaddressed everyday pass (전진한다·둘러본다·한숨 돌린다·자세를 가다듬는다·기다린다) as a chain part too** — dropping it as a single engine action skips narrate and the second intent never reaches the body. Fluff modifiers (adverbs/adjectives only, e.g. "검을 든다 (조심스레)") are not chain — chain only when an independent verb phrase exists. |
| 15 | recruit | `{"action":"recruit","target":"<npc_id>"}` | 플레이어가 NPC를 일행으로 영입하려는 의도. 신호어: "함께 가자", "동료가 되어줘", "내 일행으로", "같이 다니자". 거부 케이스(아래 § 동료 거부 규칙)에 해당하면 `{"action":"pass","targets":["<npc_id>"]}`. 굴림은 flow에서 자동 발사 (CHA, base DC 12, 호감도가 깎음). |
| 16 | dismiss | `{"action":"dismiss","target":"<companion_id>"}` | 플레이어가 동료를 일행에서 내보내려는 의도. 신호어: "보내주세요", "이제 헤어지자", "혼자 가십시오". target은 현재 companions 목록에 있어야 함. 굴림 없이 즉시 처리. |
...
**§ 동료 거부 규칙**: `recruit` 시도가 다음에 해당하면 `{"action":"pass","targets":["<npc_id>"]}`로 흡수 (narrate가 in-world 거절을 묘사):
- target NPC가 적대적 (`relations[player] < 0`) — 굴림 의미 없음
- target NPC가 `protected` (어린이/의뢰자/무력한 민간인) — 모험 동행 부적절
- target이 이미 player의 companions 목록에 있음 — 중복
- player.companions 길이 == `companions_max` — 한도 초과 ("먼저 한 명을 보내주세요")

`dismiss` 시도가 target이 companions에 없으면 `{"action":"pass","targets":["<companion_id>"]}`로 emit.
...
| Two engine branches, one is a chain-forbidden phase (rest·flee·roll·reject·summon_combat·recruit·dismiss) ("약초 마시고 상인을 설득한다" → `use` alone) | The **first verb**'s action only. Leave the second intent to narrate. **Combat is not in this list** — equip+combat / move+combat must chain with combat at the tail (see Action priority #14) |
...
`recruit` / `dismiss` (`entities=[edric_id("에드릭", friendly=true), child_01("미라", protected=true)]`, `companions=[]`, `companions_max=3`):

| Input | Output |
|---|---|
| 에드릭, 함께 가자 (에드릭 friendly, companions에 없음, party not full) | `{"action":"recruit","target":"edric_id"}` |
| 에드릭, 이제 헤어지자 (에드릭 in companions) | `{"action":"dismiss","target":"edric_id"}` |
| 에드릭, 함께 가자 (에드릭 protected) | `{"action":"pass","targets":["edric_id"]}` (companion guard — protected) |
