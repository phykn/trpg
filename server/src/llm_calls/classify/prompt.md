...
| 15 | recruit | `{"action":"recruit","target":"<npc_id>"}` | 플레이어가 NPC를 일행으로 영입하려는 의도. 신호어: "함께 가자", "동료가 되어줘", "내 일행으로", "같이 다니자". 거부 케이스(아래 § 동료 거부 규칙)에 해당하면 `reject`. 굴림은 flow에서 자동 발사 (CHA, base DC 12, 호감도가 깎음). |
...
**§ 동료 거부 규칙**: `recruit` 시도가 다음에 해당하면 `reject`로 흡수:
...
`dismiss` 시도가 target이 companions에 없으면 `reject`.
