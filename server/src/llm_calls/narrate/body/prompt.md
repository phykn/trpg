...
- `surroundings` — current location, entities, inventory, equipment, skills, growth, merchants, corpses, recent_npc, in_combat.
  ...
  - **NPC entry shape**: `{id, name, type:"npc", gender?, race?, role?, friendly?, protected?, roles?, carryables?}`. `gender` is `"male"`/`"female"` (omitted when unknown); `race` is the resolved race name; `role` is a short archetype string (정찰병/촌장/노파); `friendly: true` (omitted otherwise) marks NPCs whose affinity to player has crossed the friendly threshold — a tone hint for warmth/familiarity in dialogue, not a separate dialogue branch; `carryables: [{id, name}]` lists items the NPC could give/swap (engine-routed via judge `give`/chain — body never narrates the transfer itself). `roles` (plural) is a separate functional-flag list (`merchant`/`quest_giver`); do not confuse the two.
