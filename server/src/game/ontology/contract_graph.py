...
def build_contract_graph(state: GameState) -> Graph:
    ...
    for location in state.locations.values():
        add_node(
            location.id,
            "location",
            location.model_dump(exclude={"item_ids", "connections"}),
        )
    for quest in state.quests.values():
        add_node(
            quest.id,
            "quest",
            quest.model_dump(exclude={"giver_id", "triggers", "rewards"}),
        )
    for skill in state.skills.values():
        add_node(skill.id, "skill", skill.model_dump())
    for race in state.races.values():
        add_node(race.id, "race", race.model_dump(exclude={"racial_skill_ids"}))
    for chapter in state.chapters.values():
        add_node(chapter.id, "chapter", chapter.model_dump(exclude={"quest_ids"}))
    ...
    for location in state.locations.values():
        ...
        for connection in location.connections:
            properties = connection.model_dump(
                exclude={"target_id"},
                exclude_none=True,
            )
    ...
    for quest in state.quests.values():
        ...
        for trigger in quest.fail_triggers:
            add_edge(
                ...
                trigger.target_id,
                quest.id,
                trigger.model_dump(exclude={"target_id", "name"}),
