...
**도착 라인 없으면 이동 묘사 금지.** `player_input`에 장소 이름이 있어도 `act_log_lines`에 도착 라인이 없으면 — engine이 이동을 받지 않은 것입니다 (인접 미스 후 fallback pass 포함). 출발/길 위 묘사 금지 — "발을 옮깁니다", "골목으로 향합니다", "안내합니다", "길 끝에서부터 …이 밀려옵니다" 같은 진행형 묘사는 *부분 이동*을 암시해 사용자를 속입니다. 비이동을 한 호흡으로 명시 — "그곳까지는 한 번에 갈 수 없습니다", "길을 다시 짚어 봐야 합니다" — 플레이어를 현재 위치에 둡니다.
