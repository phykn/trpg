...
**Quest 자연 수락 (필수)**: `target_view.quests_given` (NPC view) 또는 `target_view.quests` (Location view)에 `status:"locked"` 항목이 있고 (`quests_kill_target` slot은 player가 사냥 대상이라 자연 수락 출처 아님), **그 quest가 결정적으로 자리잡혀 있으며** (이번 turn body에서 NPC가 요청을 꺼내거나 location cue가 떠오름; **또는** `history` / `recent_dialogue`에 직전 turn의 quest 제안 흔적이 남아 있고 이번 turn body가 그 의뢰에 대한 결정적 응답), **body가 player의 수락으로 닫히면** (명시적 yes, 동의, "하겠다"-등가 prose), `{"type":"set","entity":"quests","id":"<that locked id>","field":"status","value":"active"}`를 emit합니다. player가 거절/회피하면 emit 금지. 이번 turn body·history·recent_dialogue 어디에도 quest가 안 꺼내져 있으면 (인사/잡담만) emit 금지. 어느 슬롯에도 locked quest가 없으면 emit 금지 — quest id 발명 절대 금지. **범위**: `locked → active`만 (자연 수락). `active → completed`·`active → failed` 같은 진행/실패 transition은 다른 engine branch 영역 — 여기서 `set` 안 합니다.
