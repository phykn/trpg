...
- `surroundings` — id-validating slots only: `entities` (alive NPCs only), `corpses` (dead NPCs), `merchants`. Trimmed to id-validating slots only — narrower than what body sees. Use these only to anchor `state_changes`/`affinity` to real ids, never to invent new state changes.
