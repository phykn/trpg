...
| `speak` | `intent: friendly\|hostile\|deceptive\|recruit\|part\|accept\|abandon` | `target`, `tail_intent` | — | 위협→hostile, 거짓말→deceptive, 영입→recruit(`target=npc_id`), 이별→part(`target=companion_id`), 의뢰/quest 수락·맡음→accept(`target=npc_id`), 의뢰/quest 포기·그만둠→abandon(`target=npc_id`), 그 외 톤 친근→friendly |
...
| "이 일은 못하겠다 포기한다" / "관둔다" (active quest의 giver와 대화) | `[speak(intent=abandon, target=<npc_id>)]` |
