...
from src.game.engines.quest import apply_judge_result
from src.game.flow.turn import end_turn_quest_check
...
def test_active_quest_judge_satisfied_completes_quest(monkeypatch):
    from src.game.domain.memory import TurnLogEntry
...
def test_npc_dialogue_matching_giver_triggers_judge(monkeypatch):
    ...
    monkeypatch.setattr(
        "src.game.flow.narrate.judge_quest_progress", mock_judge, raising=False
    )
...
async def test_stream_narrate_tail_calls_npc_dialogue_check_for_npc_target(
    ...
    from src.game.flow.narrate import stream_narrate_tail
