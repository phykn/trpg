...
from src.game.engines.combat import (
    AttackOutcome,
    advance_turn,
...
def primary_stat_for_weapon_for_range(r: float) -> str:
    ...
    from src.game.engines.combat import _Weapon
...
def test_pick_target_nearest_returns_first_in_pool():
    ...
    assert (
        pick_target(actor, pool, graph=_graph_for(actor, first, second, third)).id
        == "first"
    )
...
def test_pick_target_healer_first_prefers_heal_skill_holder():
    ...
    assert (
        pick_target(
            actor,
            pool,
            {"heal_01": healer_skill},
            graph=_graph_for(actor, tank, healer),
        ).id
        == "doc"
    )
...
def test_pick_target_no_behavior_falls_back_to_first():
    ...
    assert (
        pick_target(
            actor, pool, rng=random.Random(0), graph=_graph_for(actor, first, second)
        ).id
        == "first"
    )
