...
@pytest.mark.parametrize(
    ...
    [
        (
            {"command": "attack", "target": "enemy_01"},
        ...
        (
            ...
            Action(verb="attack", what="enemy_01", with_="basic_strike"),
        ),
...
def test_rejects_wrong_target():
    with pytest.raises(CombatCommandError, match="target is not active enemy"):
        build_combat_command_action(
            _runtime(),
            {"command": "attack", "target": "enemy_02"},
        )


@pytest.mark.parametrize("command", ["skill", "charge"])
