...
from src.flow.skill_recommend import _build_input  # type: ignore[attr-defined]
...
def _state_with_player_skill() -> GameState:
    ...
    state.skills["s_heal"] = Skill(
        id="s_heal", name="치유", type="heal", target="self", primary_stat="WIS",
        description="자가 치유", special_effect="warm glow",
    )
    return state
