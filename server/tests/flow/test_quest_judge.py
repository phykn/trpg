...
def test_satisfied_when_history_supports_claim(monkeypatch):
    ...
    def mock_call(*args, **kwargs):
        return {"outcome": "satisfied", "reason": "처치 기록 확인", "progress_delta": None}
    monkeypatch.setattr("server.src.flow.judge._call_judge_llm", mock_call, raising=False)
    quest = {"id": "q1", "objective_text": "산문 길 약탈자 처치"}
    history = [{"type": "combat", "summary": "고블린 약탈자 처치 (HP 0)"}]
    npc_context = {"npc_id": "edric", "favor": 0}
    result = judge_quest_progress(quest, history, claim="처치했습니다", npc_context=npc_context)
...
def test_rejected_when_no_evidence(monkeypatch):
    ...
    monkeypatch.setattr("server.src.flow.judge._call_judge_llm", mock_call, raising=False)
    quest = {"id": "q1", "objective_text": "산문 길 약탈자 처치"}
    result = judge_quest_progress(quest, history=[], claim="처치했습니다", npc_context={"npc_id": "edric", "favor": 0})
