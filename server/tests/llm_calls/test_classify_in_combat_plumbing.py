...
from src.llm_calls.classify.runner import classify
...
async def test_in_combat_true_allows_move_without_destination():
    ...
    with patch("src.llm_calls.classify.runner.run_with_retries",
...
async def test_in_combat_false_rejects_move_without_destination():
    ...
    from src.llm_calls.classify.errors import ModifierValidationError
...
def test_surroundings_includes_in_combat_key():
    """context/surroundings.py가 in_combat 키를 채우는지 inspection."""
    import inspect
    from src.context import surroundings as surr_module
...
def test_classify_runner_extracts_in_combat_from_surroundings():
    ...
    import inspect
    from src.llm_calls.classify import runner as runner_module
