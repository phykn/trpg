...
6. `types/game.ts` — canonical domain model. `LogEntry` is a discriminated union over `kind: 'gm' | 'player' | 'act' | 'roll'` and drives rendering in `components/log/LogItem.tsx`. Add a new log variant by extending this union + adding a case there. Domain shapes (`Hero`, `InventoryItem`, `Memo`, `Subject`, `Quest`, `Place`) use short field names (`n`/`q`/`eq`, `t`/`m`) — keep that convention when extending.
