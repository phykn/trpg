...
7. `types/` — `domain.ts` (backend-facing: `Hero`, `Subject`, `LogEntry`, ...) and `ui.ts` (frontend-only panel contracts + `Tone`). `LogEntry` is a discriminated union over `kind: 'gm' | 'player' | 'act' | 'roll'` — extend the union and add a case in `components/log/LogItem.tsx`.
