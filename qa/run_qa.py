...
Usage (repo root 또는 어디서나):
    python qa/run_qa.py --agent diplomat --turns 15
    python qa/run_qa.py --agent all --turns 20 --profile default
"""
...
# back/src 의 모듈을 import 할 수 있도록 + run_api.py 도 import 가능
sys.path.insert(0, str(ROOT / "back"))
sys.path.insert(0, str(ROOT))

from dotenv import load_dotenv  # noqa: E402

load_dotenv(ROOT / "back" / ".env")

from src.llm_client.client import LLMClient  # noqa: E402

from qa.harness.agent import PlayerAgent  # noqa: E402
from qa.harness.review import (  # noqa: E402
    Verdict,
    review_session,
    write_review_md,
    write_verdict_json,
)
from qa.harness.runner import run_qa_session  # noqa: E402

AGENTS = ["diplomat", "explorer", "provocateur"]


def _agent_prompt_path(name: str) -> Path:
    return ROOT / "qa" / "agents" / f"{name}.md"


def _reviewer_prompt_path() -> Path:
    return ROOT / "qa" / "agents" / "reviewer.md"
...
async def main_async(args: argparse.Namespace) -> None:
    ...
    profile_dir = (ROOT / "back" / "config" / "profiles").resolve()
