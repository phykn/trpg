...
def attempt_rest(
    ...
    *,
    ...
    인카운터 발동 + sleep_encounters 풀이 비어 있으면 fallback to 풀회복 (LLM 즉석
    생성은 후속 phase). encounter 반환 시 enemy_ids 는 새로 추가된 (state 에 없는) id 가
    있다면 호출자가 알아서 등록 — 현 구현은 시드에 박혀 있는 enemy 를 가정.
    """
