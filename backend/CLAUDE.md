...
9. `errors.py` — `DomainError` + 서브클래스 (CombatNotSupported, PendingCheckActive, JudgeMalformed, PersistenceFailed, ProfileNotFound, RaceNotFound, ...). 발견한 layer 가 raise, 응답을 만드는 boundary 가 catch.
