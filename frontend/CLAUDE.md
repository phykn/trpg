...
Reference for Claude Code when working under `frontend/`. User-facing setup and tests live in [README.md](./README.md); the backend boundary contract is in `../docs/04-boundary.md`.

## Layout

`frontend/` is the Expo app. The backend is a peer directory (`../backend/`). Run all frontend commands from here.
...
- **Backend communication uses expo/fetch.** Standard `fetch` doesn't support SSE body streaming on RN.
...
7. `services/` — **data boundary**. `services/index.ts` re-exports from `services/api.ts`. The surface is `listProfiles / getCurrentSession / initSession / streamTurn / streamRoll / streamIntro`. The backend base URL and basic auth live here only — no other layer calls `fetch` directly.
8. `presenters/` — **domain → UI presenter**. `panels.ts` converts `domain` types (`Subject / Quest / Place`) into the `PanelSlot` render contract (`buildPanelSlots`). `format.ts` holds shared display helpers (`joinList`, `joinOrDash`, `formatInventoryItem`, `joinInventory`). Pure mapping — no state, no IO.
9. `types/` — `domain.ts` (backend game models: `Hero`, `Subject`, `Quest`, `Place`, `Stats`, `RollResult`, `FrontState`), `ui.ts` (frontend render contracts: `LogEntry`, `Panel`, `PanelSlot`, `Tone`, ...), `wire.ts` (backend wire formats: `ProfileCard`, `RaceCard`, `InitRequest`, `SessionPayload`, `TurnRequest`, `PendingCheck`, `JudgeAction`, `StreamEvent`). The SSE `log_entry` payload reuses `ui.LogEntry` directly. `LogEntry` is a `kind: 'gm' | 'player' | 'act' | 'roll'` discriminated union — when extending the union, add the case to `components/log/LogItem.tsx`.
...
- Frontend types only carry fields the UI renders. Backend-only fields are added during backend work.
...
- Korean only. The backend builds every display string in Korean (dates, durations, composed strings included). The frontend renders verbatim.
