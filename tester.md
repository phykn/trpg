...
4. 최대 HP +1 또는 최대 MP +1을 선택한다.

기대값: 살피기는 pending roll을 만들고, 주사위 처리 뒤 로그가 남는다. 레벨업은 시작 경험치 `1`로 바로 가능하다. 첫 레벨업 선택지는 최대 HP +1 또는 최대 MP +1을 적용하고, 레벨은 2가 되며 선택한 최대 자원만 1 오른다.
