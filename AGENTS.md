...
The persistence seam — `SaveRepo` / `ScenarioRepo` Protocols in `server/src/db/repo.py` — exists so the storage layer can be swapped without touching `game/flow/`, `llm/context/`, or `game/engines/`. `asyncio.Lock` save serialization is single-process only; multi-isolate deploys would need DB-level locks (`SELECT ... FOR UPDATE`).
