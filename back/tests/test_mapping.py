...
def _full_state(fresh_state):
    ...
    s.locations["plaza_01"] = Location(
        id="plaza_01", name="광장", tags=["outdoor"], weather=["맑음"],
        connections=[Connection(target_id="gate_01")],
    )
    s.locations["gate_01"] = Location(id="gate_01", name="성문")
    s.items["sword_01"] = Item(id="sword_01", name="검", effects=WeaponEffect(type="weapon", weapon_dice="1d8"))
    s.items["herb_01"] = Item(id="herb_01", name="약초")
    s.characters["player_01"] = Character(
        id="player_01", name="주인공", race_id="human", job="도적", level=2,
        stats=Stats(STR=12, DEX=14, CON=10, INT=10, WIS=11, CHA=13),
        hp=18, max_hp=20, mp=15, max_mp=15,
        location_id="plaza_01",
        equipment=Equipment(leftHand="sword_01"),
        inventory_ids=["herb_01", "herb_01"],
        status=["굶주림"],
        memories=[
            Memory(content="뇌물 줘서 통과", importance=2, turn=1, target_id="guard_01"),
            Memory(content="광장 노래 들음", importance=1, turn=2, target_id="plaza_01"),
        ],
        companions=["companion_01"],
    )
    s.characters["guard_01"] = Character(
        id="guard_01", name="경비병", race_id="human", job="경비", role="마을 경비병",
        appearance="갑옷의 중년", stats=Stats(),
        hp=20, max_hp=20, relations={"player_01": 30},
    )
    s.characters["companion_01"] = Character(
        id="companion_01", name="펭", race_id="goblin", job="", stats=Stats(),
    )
    s.quests["q1"] = Quest(
        id="q1", title="t", summary="진행 중", giver_id="guard_01", difficulty="어려움",
        triggers=[
            QuestTrigger(id="a", name="처치", type="character_death", target_id="goblin_01"),
            QuestTrigger(id="b", name="보고", type="location_enter", target_id="plaza_01"),
        ],
    ...
    s.log_entries = [
        ...
        RollLogEntry(id=2, kind="roll", check="x", dc=10, roll=14, mod=2, result="success"),
    ]
    return s
...
def test_inactive_slots_return_none(fresh_state):
    fresh_state.characters["player_01"] = Character(
        id="player_01", name="x", race_id="human", stats=Stats(),
    )
...
def test_no_internal_field_leakage(fresh_state):
    ...
    forbidden = [
        "disposition", "tone_hint", "memories", "location_id", "relations",
        "combat_behavior", "triggers", "inventory_ids", "racial_skills",
        "learned_skills", "active_buffs", "xp_pool", "death_saves",
        "race_id", "giver_id", "is_player", "fail_triggers",
    ]
    for f in forbidden:
...
def test_unknown_race_id_falls_back_to_id(fresh_state):
    fresh_state.characters["player_01"] = Character(
        id="player_01", name="x", race_id="unknown_race", stats=Stats(),
    )
