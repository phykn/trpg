...
def state(fresh_state):
    fresh_state.locations["plaza_01"] = Location(id="plaza_01", name="광장", item_ids=["stone_01"])
    ...
    fresh_state.characters["player_01"] = Character(
        ...
        location_id="plaza_01", inventory_ids=["key_01"],
    )
    fresh_state.characters["guard_01"] = Character(
        id="guard_01", name="경비", race_id="human", stats=Stats(),
        location_id="plaza_01",
    )
    fresh_state.chapters["ch1"] = Chapter(id="ch1", title="t", status="active")
    fresh_state.quests["q1"] = Quest(
        id="q1", title="t", giver_id="guard_01", difficulty="보통",
        triggers=[QuestTrigger(id="x", name="n", type="character_death", target_id="goblin_01")],
        status="active",
    )
    return fresh_state
...
def test_set_scalar_and_dotted(state):
    r = apply_changes(state, [
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "tone_hint", "value": "격식"},
...
def test_set_engine_owned_field_rejected(state):
    r = apply_changes(state, [
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "hp", "value": 0},
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "level", "value": 99},
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "memories", "value": []},
    ])
...
def test_set_chapter_quest_only_summary_or_status(state):
    r = apply_changes(state, [
        {"type": "set", "entity": "quests", "id": "q1", "field": "summary", "value": "x"},
        {"type": "set", "entity": "chapters", "id": "ch1", "field": "status", "value": "completed"},
        {"type": "set", "entity": "quests", "id": "q1", "field": "difficulty", "value": "신화"},
        {"type": "set", "entity": "chapters", "id": "ch1", "field": "title", "value": "X"},
    ])
...
def test_move_valid_and_unknown(state):
    r = apply_changes(state, [{"type": "move", "target": "player_01", "destination": "gate_01"}])
    assert r["applied"] == 1 and state.characters["player_01"].location_id == "gate_01"
    r = apply_changes(state, [{"type": "move", "target": "player_01", "destination": "nowhere"}])
    assert r["applied"] == 0


def test_move_item_between_containers(state):
    r = apply_changes(state, [{"type": "move_item", "item": "key_01", "from": "player_01", "to": "plaza_01"}])
    assert r["applied"] == 1
...
def test_affinity_grade_intent_matrix(state):
    apply_changes(state, [{
        "type": "affinity", "actor": "player_01", "target": "guard_01",
        "grade": "success", "intent": "friendly",
    }])
    ...
    apply_changes(state, [{
        "type": "affinity", "actor": "player_01", "target": "guard_01",
        "grade": "success", "intent": "deceptive",
    }])
    assert state.characters["player_01"].relations["guard_01"] == 0  # 속임수 성공 = 0

    state.characters["player_01"].relations["guard_01"] = 0
    apply_changes(state, [{
        "type": "affinity", "actor": "player_01", "target": "guard_01",
        "grade": "failure", "intent": "deceptive",
    }])
    ...
    apply_changes(state, [{
        "type": "affinity", "actor": "player_01", "target": "guard_01",
        "grade": "critical_success", "intent": "friendly",
    }])
...
def test_partial_success_keeps_valid_changes(state):
    r = apply_changes(state, [
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "tone_hint", "value": "x"},
        {"type": "set", "entity": "characters", "id": "guard_01", "field": "hp", "value": 0},
