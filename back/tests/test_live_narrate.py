...
async def test_narrate_roll_success(client):
    input_ = NarrateInput(
        ...
        history="",
        target_view={
            "type": "npc", "name": "경비병", "tone_hint": "격식체",
        ...
        judge_result={"action": "roll", "tier": "보통", "stat": "CHA", "targets": ["guard_01"]},
        grade="success",
        player_input="경비병에게 동전을 쥐여주며 통과시켜달라고 한다",
    )
    body, final = await _collect(stream_narrate(client, input_))
    assert len(body) > 20  # 본문 어느 정도
    assert final.output.turn_summary  # 비어있지 않음
...
async def test_narrate_reject_engine_forces_clean_output(client):
    ...
    body, final = await _collect(stream_narrate(client, input_))
    assert len(body) > 0
    # 본문에 "알 수 없는 힘", "현기증" 같은 인-게임 흡수 표현이 들어가는 경향이 있지만
    # 모델 변동성이 있어서 본문 내용은 검증 안 함. state_changes 가 비어 있어야 함.
