...
def env():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        gs = GameState(
            game_id="t", profile="default", player_id="player_01",
        ...
        gs.characters["player_01"] = Character(
            ...
            location_id="plaza_01", hp=20, max_hp=20, mp=15, max_mp=15,
        )
        gs.characters["guard_01"] = Character(
            id="guard_01", name="경비병", race_id="human", stats=Stats(),
            location_id="plaza_01", appearance="갑옷", tone_hint="격식체",
        )
        yield gs, str(profile_dir), str(data_dir)
...
async def test_pass_branch_full_flow(client, env):
    gs, profile_dir, data_dir = env
    events = await _collect_events(run_turn(
        client, gs, profile_dir, data_dir, "주변을 둘러본다.",
    ))
...
async def test_roll_branch_pauses_then_resumes(client, env):
    gs, profile_dir, data_dir = env
    events = await _collect_events(run_turn(
        client, gs, profile_dir, data_dir,
    ...
    with pytest.raises(PendingCheckActive):
        async for _ in run_turn(client, gs, profile_dir, data_dir, "..."):
            pass

    # /roll 진행
    roll_events = await _collect_events(run_roll(
        client, gs, profile_dir, data_dir, rng=random.Random(7),
    ))
    assert roll_events[0]["type"] == "log_entry"  # roll log
    assert roll_events[-1]["type"] == "done"
    assert gs.pending_check is None
    # turn_count 는 /roll 에서 +1 (roll 분기는 /turn 에선 안 올림)
    assert gs.turn_count == 1
...
async def test_roll_without_pending_blocked(client, env):
    ...
    with pytest.raises(PendingCheckExpected):
        async for _ in run_roll(client, gs, profile_dir, data_dir):
            pass
