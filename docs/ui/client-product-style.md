## TRPG Client Product Style

This section adapts the warm editorial reference below to the actual TRPG play client. The app is a compact Korean text-TRPG surface, not a marketing page and not an illustrated RPG inventory screen.

### Visual Direction

- The outer app canvas uses warm cream, while the playable phone surface stays dark warm ink.
- The main accent is muted coral/salmon. Use it for the active top pill, GM narration stripe, selected dice threshold, and primary action buttons.
- Keep shadows minimal. Depth should come from dark surface steps, 1px hairlines, and spacing.
- Story prose uses the serif stack. UI labels, buttons, stats, and panel metadata use the sans stack. Numeric stat values use the mono stack with tabular numbers.
- Avoid decorative fantasy frames, parchment textures, large character art, scene backgrounds, emoji, and ornamental icons.
...
- The composer owns the terse nearby summary line: `인물 N · 장소 N · 할 일 N`.
- Do not add a separate `열기` button. The summary line itself is the tap target.
- When expanded, the nearby panel sits above the composer without covering the input row.
- Nearby rows show the thing name, a short factual line, and a right-side action button when the action is available.
- Remove bottom “quick action” suggestion strips from the log. Action shortcuts belong in nearby or in the active combat/roll panel.
...
- All player-visible text stays Korean.
- Do not add explanatory UI copy unless it is required to perform the action.
- GM narration and player entries use the same large log rhythm; distinguish them by border side/color and text color, not by chat bubbles or labels.
- Functional controls only. If a button does not trigger an action in the current state, remove it.
