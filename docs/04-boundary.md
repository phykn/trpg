...
| `reviveCoins`, `reviveCoinsMax` | `Character.revive_coins` (현재 잔량) + `RULES.death.revive_coins` (init 시 부여 = max) | revive 토큰 미터. 토큰이 있으면 0 HP 직격 시 death save 분기 대신 `auto_revive_hp` (기본 1) 로 즉시 부활하고 1 깎임 ([03-features.md](./03-features.md) §1.4). 프론트 패널이 미터로 렌더 |
...
active_subject_id 는 죽은 NPC 도 가리킬 수 있다 — *subject pin* 이라 부르는 `state.active_subject_id` 의 현재 지목 (`flow/subject.py:refresh_active_subject` 가 combat / roll / trade / 최근 대화 NPC 로 동기화) 이 사망 시점에 사라지지 않고 corpse-aware 로 살아남아, narrate 가 죽은 정체성을 anchor 로 잡고 회상·시체 묘사를 잇는다. corpse 일 때 `known` 한 줄 (`["죽음"]`) 만 채워지고 나머지 수치는 사망 직전 상태 그대로 노출.
...
| `targets: PlaceTarget[]` | 같은 location 에 있는 player 외 캐릭터 | `{name, level, raceJob, gender, blurb, trust}`. `level/raceJob/gender` 규칙은 Hero 와 동일. `blurb` = 살아 있으면 `appearance` (없으면 `description`), 죽어 있으면 `"죽음"`. `trust` = `relations.get(player_id, 0)`. 시체도 포함 — `level` 등 수치는 사망 직전 값이 그대로 노출 (engine 이 죽을 때 reset 안 함), 패널이 corpse 톤으로 흐림 처리. |
...
- **Character.role: str** — 자유 문자열 ("몬스터", "마을 장로" 등). NPC 시드 (`scenarios/{id}/characters/*.json`) 에 들어 있는 값.
...
- **Location.connections: list[Connection]** — 인접 장소 엣지 (`target_id`, `difficulty`, `key_item_id`). `Place.surroundings` 가 각 엣지를 `{name, blurb, difficulty}` 로 사영 — `difficulty` 는 위 표(§1 Place) 의 라벨 변환 규칙을 따른다.
...
| 메서드 | 경로 | 바디 | 응답 |
|---|---|---|---|
| GET  | `/health` | — | `{ok: true}`. Basic Auth 없이 호출 가능한 유일한 endpoint |
...
| POST | `/session/{id}/turn` | `{player_input: string, think?: bool}` | `text/event-stream` (SSE — 서버가 한 연결을 열어둔 채 이벤트를 계속 흘려주는 방식). `think` 는 LLM thinking 토글 (기본 false, OPT/OPT_ON 카테고리 모델에만 효과 — [02-runtime.md](./02-runtime.md) §1.3) |
| POST | `/session/{id}/roll` | `{think?: bool}` (서버가 d20 굴림) | `text/event-stream`. `pending_check.kind="stat"` 의 일반 굴림만 받음 — 전투/죽음 굴림은 자동 처리되어 dice 버튼이 뜨지 않음 ([02-runtime.md](./02-runtime.md) §2.2). `think` 는 turn 과 동일 |
| POST | `/session/{id}/intro` | — | `text/event-stream`. 게임 시작 직후 첫 GM narration 한 번. judge 안 부르고 `flow/intro.py` 가 narrate 만 호출 |
| POST | `/debug/complete` | `{system?: string, query: string, think?: bool}` | `{think, answer}`. 디버그 전용 — LLM 한 번 호출해 raw 응답 확인 (Basic Auth 필요) |

`FrontState = {hero, subject, quest, place, combat, log, pendingCheck, storyGraph}`. `storyGraph` 는 플레이어 시점 가시 영역 (현재 location 의 reachable map + 현장 NPC + companions + active subject) 만 담은 `{nodes, edges, summary}` 묶음으로, `to_story_graph` 에서 조립된다 — 시나리오 전체 dump 가 되지 않게 의도적으로 좁힘. 노드별 필드 (level/raceJob/risk/dayPhase/...) 는 패널이 그대로 렌더하도록 백엔드에서 합성된다. SSE `state` 이벤트 ([02-runtime.md](./02-runtime.md) §2.4) 는 같은 8 슬롯 묶음 — `log` 는 누적되는 흐름이라 `log_entry` 이벤트로도 따로 흐른다 (state 안에는 영속본 꼬리만). **`FrontState.log` 영속본 cap 은 `rules.log.display_turns` (기본 20)** — `GET /session/{id}/state` 는 최근 20 턴치만 반환 ([02-runtime.md](./02-runtime.md) §6.2 디스플레이 로그 영속화). `pendingCheck` 는 `state.pending_check` 가 활성일 때만 채워짐 — 앱이 굴림 도중에 닫혀도 다음 GET 으로 UI 가 복원된다.
...
세션 흐름 — 앱 시작 시 클라이언트 `localStorage` 에 보관된 game_id 가 있으면 `GET /session/{id}/state` 로 복원 시도 → 200 이면 진행 중 게임 복원, 404·미보관이면 `GET /profiles` 호출 → 프론트가 시나리오·종족 카드를 보여주고 사용자가 캐릭터 생성 → `POST /session/init` 호출 (응답의 game_id 를 localStorage 에 저장) → `POST /session/{id}/intro` 로 시작 GM narration → 이후 `/turn` 으로 진행. 사용자 한 명당 한 게임 흐름이고 다중 사용자 안전성 invariant 는 [02-runtime.md](./02-runtime.md) §2.5 참조. 게임 목록·이어하기 화면은 P1 에 없음.
...
**`game_id` 검증 범위** — 아래 §3 첫 행의 404 매핑은 `Depends(get_state)` 한 곳에서 일어나므로 `/session/{id}/state` · `/turn` · `/roll` · `/intro` 등 game-스코프 라우트가 모두 같은 동작을 공유한다 (한 라우트만의 특수 동작 아님).
