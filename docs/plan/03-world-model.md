...
LLM은 관계 숫자를 직접 쓰지 않는다. 관계 변화가 필요하면 engine이 먼저 정하고, LLM은 정해진 결과를 이야기로 쓴다.
...
`graph`는 게임 데이터의 원천이다. engine, LLM 후보 목록, 화면 데이터는 모두 graph를 기준으로 만든다.
...
LLM에게 전체 graph를 주지 않는다. server가 지금 장면에 필요한 후보 목록만 만들어 준다.
...
`profile.json`과 `world.md`는 graph 밖의 profile 설정으로 둔다. LLM 후보 목록에는 들어갈 수 있지만, 캐릭터나 아이템처럼 게임 중 움직이는 node는 아니다.
