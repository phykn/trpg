...
- **공격 굴림**: 라운드별 actor 마다 `engines/combat.attack` 이 d20 + stat_modifier vs `enemy_defense` 로 명중·등급 결정. weapon dice + stat_mod 로 데미지. 어떤 스탯을 쓸지는 무기로 갈림 — 사거리 1.5m 이하 근접 무기는 STR, 그보다 멀면 DEX. 무기 없는 맨손은 `RULES.combat.unarmed.damage` / `.range_m` 폴백.
...
`rules.death` 가 사망 처리 룰을 모은다 (`Rules` 의 직접 필드 — `rules.combat` 의 sibling).

- `instant_death: bool = False` — 전역 플래그. True 면 플레이어도 HP 0 이 되는 순간 즉시 사망. NPC·몬스터는 이 플래그와 무관하게 `engines/combat.apply_attack_to_defender` 의 `not is_player` 분기로 자동 즉사 (현재 line 414).
- `revive_coins` — 플레이어 전용 목숨 토큰. 캐릭터 모델 기본값은 `0` (`Character.revive_coins: int = 0`, `domain/entities.py:214`) 이라 NPC·몬스터는 토큰을 갖지 않지만, **신규 플레이어 init 시 `persistence/init.py` 가 `RULES.death.revive_coins` (기본 3) 를 세팅**해 첫 게임부터 3 개를 들고 시작한다. 토큰이 1 이상일 때 0 HP 직격이 들어오면 `apply_attack_to_defender` 의 즉시 부활 분기 (`engines/combat.py:406-407`) 가 발동해 토큰 1 을 깎고 `RULES.death.auto_revive_hp` (기본 1) 로 회복. 토큰이 0 일 때만 death save 분기로 떨어진다. 프론트는 `reviveCoins` / `reviveCoinsMax` 두 필드로 미터를 받는다 ([04-boundary.md](./04-boundary.md) §1 Hero, `mapping/to_front.py` 의 `to_hero`).
...
- `combat_turn`: 자동 사이클 안에서 한 actor 의 한 행동 상세. payload 8-op enum 과 필드 정의는 [02-runtime.md](./02-runtime.md) §2.4 가 SSOT — UI 는 보통 무시 (정확한 상태는 `state` 와 `log_entry` 에 실림), 테스트가 관찰 가능 신호로 사용.
...
레거시 슬롯 (`head/left_hand/right_hand/chest/legs/necklace/ring_1/ring_2`) 은 폐기 — 마이그레이션 shim 없음, 시드·세이브 모두 3 슬롯 모양으로만 통한다. 외부 계약 (프론트, API) 은 위 3 슬롯이 기준 ([04-boundary.md](./04-boundary.md) §1 Hero/Subject 의 `equipment` 행, [05-codemap.md](./05-codemap.md) §1 `entities.py` 의 `Equipment`).
...
**현재 구현 상태**: cast 핵심 (S1) + judge 의미 매칭 (S2) + 학습 후보 (§2.3 4단계 / S3) 까지 들어갔고 자연어 통합. judge 가 `combat` 액션의 `skill_id` 필드 (또는 의미 매칭, S2) 로 어느 기술을 쓸지 결정하고, `flow/combat_auto.py` 의 자동 사이클 안에서 `engines/skill.py:cast` 가 매 라운드 호출된다 (MP/level 부족 시 평타 폴백). 옛 `POST /cast` REST 엔드포인트는 폐기. 모든 cast 는 combat 분기를 통과해 grade 를 받아 들어온다 (out-of-combat 통로 없음, [05-codemap.md](./05-codemap.md) §3 의 SkillInvalid 흡수 경로 참고).
