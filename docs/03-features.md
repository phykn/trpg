...
**현재 구현 상태**: `engines/inventory/use.py` + `api/routes/inventory.py` (`/use` endpoint) + judge 자연어 통합. heal/damage/mp_restore/buff 분기, on_use 트리거 패스스루 (quest trigger 평가는 §2.8), consumable=True 시 인벤 차감. 자연어 "약초를 먹는다", "포션을 마신다" 같은 입력도 `surroundings.inventory` 컨텍스트로 받아 judge 가 `action="use", item_id` 로 분류 (Phase 1 자연어 통합).
