...
backend/
  ...
  src/
    ...
    domain/
      ...
      types.py                   StatKey, Tier, Grade, Intent, Action enum (pass / roll / combat / rest / use / equip / unequip / clarify / reject)
...
- `DomainError` — 모든 도메인 에러의 기반 클래스.
  ...
  - `SkillInvalid` — `/cast` 요청이 레벨·MP·사정거리·소유 검증 실패. API 가 422 로 매핑.
