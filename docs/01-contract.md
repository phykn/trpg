...
- LLM은 이야기를 쓰고, 플레이어 말을 정리한다.
- engine은 게임 규칙을 검사하고, graph를 바꾼다.
- graph가 게임 데이터의 원천이다.
- LLM이 말한 일은 engine이 받아들여야만 게임의 사실이 된다.
...
| `Action` | 플레이어가 하려는 행동 |
| `NarrateAction` | 이야기 뒤에 저장할 수 있는 작은 변화 |
...
`graph`는 게임의 원본 데이터다. engine, LLM context, 화면 데이터는 모두 graph를 기준으로 만든다.
...
`Action`은 플레이어가 하려는 일을 정리한 값이다.

`Action`에는 성공 여부가 들어가지 않는다. “고블린을 공격한다”는 들어가지만, “공격이 성공했다”는 들어가지 않는다.

`Action`은 graph를 직접 바꾸지 않는다. engine이 `Action`을 검사한 뒤 필요한 `GraphChange`를 만든다.
...
확인 전에는 graph와 진행 상태가 바뀌지 않는다. 확인하면 원래 행동을 다시 처리한다. 취소하면 확인 대기만 사라진다.

## NarrateAction

LLM이 쓴 이야기는 바로 graph를 바꾸지 않는다.

다만 이야기에서 이미 말한 작은 변화는 `NarrateAction`으로 요청할 수 있다.

허용되는 변화:

| action | 할 수 있는 일 |
|---|---|
| `set_property` | 허용된 node나 edge 속성 하나 바꾸기 |
| `place_item` | 아이템 위치 edge 만들기 |
| `remove_item` | 아이템 위치 edge 제거하기 |
| `affinity` | 관계 edge의 감정 변화 요청 |

`NarrateAction`으로 하면 안 되는 일:
...
이런 일은 engine 규칙으로만 처리한다.

`NarrateAction`은 `GraphChange`가 아니다. engine이 허용된 `NarrateAction`을 검사한 뒤 필요한 `GraphChange`로 바꾼다.
