...
전투 버튼을 만들 수는 있다. 버튼은 `Action`을 보낼 수 있지만, 성공/실패와 상태 변경은 server가 정한다.
...
| 2 | `combat` | 전투 상태를 먼저 보여줌 |
| 3 | `suggestions` | 행동 제안으로만 보여줌 |
...
| table | 역할 |
|---|---|
| `game_progress` | 현재 턴, 활성 퀘스트, 확인 대기, 전투 임시 상태 |
