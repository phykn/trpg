...
judge 의 15 종 액션 분기와 가장자리 동작 (모호 입력 → location 폴백, reject 흡수, chain, tail_intent, semantic skill 매칭 회피) 을 한 바퀴 두드립니다. 한 번 흡수되면 다른 방향으로 시도합니다 (무한 반복 금지).
...
- **스킬 매칭 회피**: 적이 같은 장소에 있어도 "맨손으로 친다", "그냥 평타로 친다" 로 스킬 매칭 끔 — 사일런트 폴백 확인.
...
| 5 | 모호 입력 (폴백) | "그래서요?", "..." |
| 6 | combat | 적대 NPC 공격. 없으면 다음 턴에서 summon_combat 으로 |
...
| 14 | 스킬 매칭 회피 | 적 앞에서 "그냥 평타로 친다" |
| 15 | reject (장르 이탈) | 명백히 현대·SF 소품 |

### 절대 규칙

- 표대로 종류를 돌린다. 같은 종류를 두 턴 연속 두드리지 않는다.
- 후반 턴에 도발 소재가 떨어지면 장소 이동으로 빠지지 말고, 표의 reject·모호·폴백 종류를 새 표현으로 다시 시도한다.
