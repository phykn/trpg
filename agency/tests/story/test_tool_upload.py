"""upload subcommand — Supabase Storage put for every file under scenario_dir.

Network is mocked: we monkeypatch _Storage to record put_bytes calls instead
...
def test_upload_uploads_every_file(capsys, tmp_path, fake_storage_env):
    ...
    with patch("agency.story.tool._Storage", return_value=fake):
        rc = tool._main(["upload", str(sd)])
