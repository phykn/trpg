...
def _scaffold_with_inventory(tmp_path: Path) -> Path:
    ...
    (sd / "items").mkdir()
    (sd / "items" / "sword.json").write_text(json.dumps({
        "id": "sword",
        "name": "철검",
        "description": "쇠로 만든 검.",
        "weight": 3,
    ...
    (sd / "items" / "leather.json").write_text(json.dumps({
        "id": "leather",
        "name": "가죽 갑옷",
        "description": "낡은 가죽.",
        "weight": 5,
    ...
    (sd / "characters").mkdir()
    (sd / "characters" / "fighter.json").write_text(json.dumps({
        "id": "fighter", "name": "검사", "description": "전사.",
        "race_id": "human", "location_id": "town", "level": 1,
        "stats": {"STR": 10, "DEX": 10, "CON": 10, "INT": 10, "WIS": 10, "CHA": 10},
        "hp": 27, "max_hp": 27, "mp": 20, "max_mp": 20,
        "racial_skill_ids": [], "learned_skill_ids": [],
        "inventory_ids": ["sword", "leather"],
        "equipment": {"weapon": None, "armor": None, "accessory": None},
        "job": "전사", "gender": "male", "alive": True,
        "xp_reward": 0,
    }, ensure_ascii=False), encoding="utf-8")
    return sd
