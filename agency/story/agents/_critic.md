...
**룰 위반은 무시.** 페어 트레이드(스탯 합 60), HP/MP 공식, equipment 슬롯-effect 매칭, skill level ≤ owner level 같은 것은 코드가 이미 검사한다. 너는 그 영역을 보지 마라.
...
ok=true 기준은 **관대하게**. 명백히 어색하거나 룰 자체로는 통과하지만 게임 진행에 의미적으로 문제가 될 만한 것만 false. 사소한 표현·취향 차이는 통과.

대부분의 entity 는 ok=true. false 는 진짜 이상한 경우만.
...
```json
{"ok": false, "feedback": "강도패 두목인데 inventory 에 약초만 있고 무기가 없다. 도적·우두머리는 검·암기 같은 무기를 inventory_ids 에 추가하고 equipment.rightHand 에 박는 게 자연스럽다. 가벼운 갑옷이나 가죽 보호구도 있으면 좋다."}
```
