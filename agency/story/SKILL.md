...
도입부 품질 기준:
- 첫 문장은 정보 요약이 아니라 보이는 물건, 막힌 길, 이상한 빈자리 중 하나로 시작한다.
- `당신은 <장소>에 서 있습니다`, `<NPC>가 규칙을 설명합니다` 같은 상태 설명형 시작 금지.
- 시작 위치의 규칙·목표·동행자 이름을 한 번에 설명하지 않는다. 물건, 막힌 길, 빈자리처럼 플레이어가 “왜?”를 묻게 만드는 단서부터 둔다.
...
catalog는 프롬프트가 아니다. 고정 id, 이름, 짧은 설명만 둔다. `금지`, `목표`, 말투 지시, 장면 운영 규칙, LLM 출력 형식 같은 지시문을 catalog에 넣지 않는다. 시나리오 고유 운영 규칙은 `chapter.guidance`, `world.md`, 공개 `knowledge`에 두고, 공통 문체·런타임 규칙은 server prompt나 `agency/qa/SKILL.md`에 둔다.
...
- 시나리오의 문제는 거창한 설정보다 플레이어가 바로 이해할 수 있는 작은 사건에서 시작한다. 예: 손수레가 진흙투성이로 돌아왔는데 사과를 못 받은 일처럼, 사소하지만 당사자에게는 억울한 일을 감정 섬의 씨앗으로 삼는다.
- 쉬운 말을 우선한다. 플레이어가 한 번 읽고 누가 무엇 때문에 화났는지, 무엇을 확인해야 하는지 알 수 있어야 한다.

### 플레이테스트 반영 기준

- 튜토리얼은 새 플레이어의 질문을 견뎌야 한다. 플레이어가 모르는 전제를 물어야만 진행되는 구조를 만들지 말고, 먼저 화면에 보이는 단서로 질문의 입구를 둔다.
- 플롯보다 상황을 준비한다. 정해진 대사 순서나 정답 루트보다, NPC의 목표, 장소의 압박, 남은 자원, 충돌 중인 이해관계, 플레이어가 건드릴 수 있는 단서를 먼저 둔다.
- 첫 갈등 장은 시나리오 품질의 기준점이다. 중심 인물, 중심 물건, 갈등의 양쪽 주장, 관찰 가능한 주변 반응을 분리해서 넣어야 대화·조사·선택 테스트가 버틴다.
- 첫 갈등 장 이후의 섬도 같은 밀도를 유지한다. 뒤 섬이라고 `핵심 상황 + 최종 선택`만 두지 말고, 플레이어가 물을 법한 원인, 현재 피해, 각 인물의 근거, 장소 단서, 선택 후 남는 것을 공개 `knowledge`, `location.traits`, `character.knowledge`, `chapter.guidance`에 나눈다.
- 사건의 핵심 물건은 소유 상태를 안정적으로 둔다. 누가 들고 있는지, 아직 플레이어에게 넘어오지 않았는지, 선택 후에만 얻는지 seed 여러 필드가 같은 말을 해야 한다.
- 검은 보상 아이템처럼 선택 후에만 얻는 물건은 선택 전 `location.items`에 넣지 않는다. `items.json`에는 정의하되, 실제 획득은 quest reward나 선택 처리로만 일어나야 한다.
- 같은 단어가 챕터, 섬, 장소를 동시에 가리키지 않게 한다. 챕터는 넓은 장 이름, location은 실제 도착 지점, connection은 이동 경로를 맡긴다.
- QA에서 나온 어색함을 고칠 때는 먼저 seed로 고칠 문제인지, runtime prompt/brief로 고칠 문제인지 가른다. 시나리오 특징은 scenario 파일에, 공통 규칙은 server prompt나 agency 기준에 둔다.
...
- 갈등의 출발점은 공개 `knowledge`에 반드시 쉬운 문장으로 둔다. 플레이어가 “왜 화났어요?”, “무슨 일이 있었어요?”라고 물으면 바로 답할 수 있어야 한다.
- 감정 섬의 원인은 너무 추상적으로 쓰지 않는다. `인정받지 못한 분노`보다 `빌려준 손수레가 진흙투성이로 돌아왔는데 사과를 못 받았다`처럼 실제 사건을 쓴다.
- 필수 결론은 한 통로에만 두지 않는다. 플레이어가 반드시 알아야 하는 정보는 NPC 답변, 장소 물건, 공개 knowledge, 퀘스트 설명 중 둘 이상에서 확인할 수 있게 한다.
- 플레이어가 특정 NPC에게 물을 법한 핵심 쟁점은 `knowledge`에 답변 재료를 쪼개 둔다. 예: `한 일`, `하지 않은 일`, `환불 이유`처럼 질문 단위로 나누면 NPC가 은유나 되묻기로 빠지지 않는다.
- 각 감정 섬에는 최소한 `시작된 이유`, `지금 문제가 된 이유`, `의뢰인의 구체적 피해`, `흰 입장의 근거`, `검은 입장의 근거`, `선택 후 남는 것`에 답하는 공개 지식이 있어야 한다. 숫자를 맞추기 위한 지식이 아니라 질문을 버티는 지식이어야 한다.
...
- 계속 동행하는 캐릭터의 `knowledge`는 언제든 답변에 섞일 수 있다. 후반 장면 전용 질문이나 스포일러는 동행자 지식에 미리 넣지 말고, 해당 `chapter.guidance`나 장소/퀘스트 재료에 둔다.
...
- 메인 루트의 필수 선택지도 주사위 성공 하나에 잠그지 않는다. `social_check`는 추가 정보, 신뢰, 더 선명한 힌트를 여는 데 쓰고, 선택 가능 상태는 핵심 장소 진입이나 명확한 관찰처럼 실패하지 않는 trigger로 연다.
...
- `location.description`은 장소만 설명한다. 특정 NPC의 행동, 표정, 대사, 직전 대화 요약은 캐릭터 위치가 바뀌면 어색해지므로 넣지 않는다. 인물 정보는 `character.location`, `character.traits`, `character.knowledge`, `chapter.guidance`에 둔다.
- `location.description`은 이동 로그에 직접 노출된다. 나레이션 본문과 이질감이 없도록 플레이어-facing 문장으로 보고, 한국어 시나리오에서는 `~합니다`, `~입니다` 현재형을 쓴다. `mood`, `traits`, `knowledge.summary`는 LLM 재료이므로 짧은 간결체를 유지해도 된다.
- 세계 설명을 NPC 대사에 몰아넣지 않는다. 장소의 배치, 닫힌 문, 빈 의자, 찢긴 표식, 남은 물건처럼 플레이어가 볼 수 있는 흔적으로도 정보를 나눠 둔다.
- 핵심 사물이 현재 장소에 있다면 description에서 바로 보이게 쓴다. 필수 동행자나 NPC의 존재는 `character.location`과 캐릭터 필드로 드러내고, 장소 description에 인물을 고정하지 않는다.
...
- quest `handoff`는 방금 끝난 사건에서 다음 사건으로 넘기는 짧은 다리다. handoff에 쓰지 않은 이전 퀘스트 물건이나 보상 아이템을 LLM이 다시 꺼내지 않게, 다음 장소·물건·쟁점만 둔다.
- handoff에서 물건을 다룰 때는 소유를 흔들지 않는다. 선택 보상을 주지 않는 루트라면 `플레이어 손`, `받는다`, `쥔다` 같은 획득 표현 대신 `게시판 아래에 남아 있다`, `식탁 위에 놓인다`처럼 위치를 고정한다.
- seed를 고쳐도 이동 로그가 계속 직전 대화를 잇는다면, seed 문제가 아니라 narration brief에 이전 대화 맥락이 들어가는 구조 문제일 수 있다.
- 이동 narration은 현재 location의 `description`/`traits`와 현재 위치의 character를 근거로 삼는다. 선착장처럼 경계가 중요한 장소는 보이는 물건, 현재 그곳에 있는 인물, 아직 일어나지 않은 도착을 명확히 갈라 쓴다.
...
- 선택 완료 뒤 다음 섬으로 나가는 장소에는 gated connection을 로컬 되돌아가기 connection보다 앞에 둔다. 자유 입력 분류기는 목적지를 말해도 첫 연결에 끌릴 수 있으므로, 메인 루트 진행 연결의 우선순위를 seed 순서로 보강한다.
- 단, connection 순서로 정답을 숨기지 않는다. 현재 장소에서 실제로 갈 수 있는 로컬 장소도 남기되, 완료 후 자연스러운 다음 루트가 먼저 잡히게 정렬한다.
...
- NPC는 GM처럼 객관적 상황을 안내하지 않는다. `knowledge`는 사실을 담고, character 필드는 그 사실을 받아들이는 입장·두려움·요구·말버릇을 담는다.
- NPC 대사 재료는 `무엇을 느끼는가`, `플레이어를 어떻게 보는가`, `무엇을 바라나`가 갈라져야 한다. 출력에서는 이 분석을 직접 말하지 않고 말투와 짧은 행동으로 드러나게 한다.
- NPC가 말할 때는 정보보다 압박을 먼저 본다. 이 인물이 지금 얻고 싶은 것, 피하고 싶은 것, 플레이어에게 요구하는 것이 없으면 대사가 안내문처럼 흐른다.

### 문체 재료

- 담백한 플레이 로그체를 유도한다. 멋있는 문장보다 짧은 동작, 거리, 자세, 반응을 seed 재료로 둔다.
- 문체 재료는 `물러남`, `비틀거림`, `붙잡음`, `놓침`, `팔을 듦`, `빈손`, `벽에 닿음`처럼 보이는 행동이어야 한다.
- 중요한 소유·위치·행동 제약은 가능하면 부정문보다 긍정 관찰문으로 쓴다. `주머니에서 꺼내지 않는다`보다 `처음부터 손에 든 영수증을 들어 보인다`가 낫다. 부정문은 금지한 단어를 나레이션에 새로 노출할 수 있다.
- 공통 프롬프트나 QA 기준에도 금지 단어를 예시로 길게 넣지 않는다. 문제 단어를 직접 노출하면 LLM이 그 단어를 새 장면에 끌어올 수 있다.
- 캐릭터의 `contradiction`은 금지된 능력처럼 읽히면 안 된다. 혼자 갈 수 없는 장면에서 `혼자 앞서가고 싶다`보다 `완벽한 안내자처럼 굴고 싶지 않다`, `자기 자리에서 노를 맞춘다`처럼 역할 안의 긴장으로 쓴다.
...
- 선택 보상 아이템은 `location.items`에 미리 배치하지 않는다. 장소에 놓이면 플레이어가 선택 전에 주울 수 있는 affordance가 되므로, 검은/흰 선택의 의미가 깨진다.
