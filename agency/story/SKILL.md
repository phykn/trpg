...
`learned_skills` (decomp), `inventory` (decomp items 중 owner_character 매칭)을 채운다. 종족 기술은 character에 복사하지 않는다 — runtime이 race의 `racial_skills`에서 `grants_skill` edge를 만든다. NPC `equipment`는 `{}`로 둔다. 장비성 아이템도 NPC에게는 `inventory`에만 넣는다.
...
- `action`은 `"precise"`, `"reckless"`, `"defend"`, `"guarded"`, `"create_distance"`, `"talk"` 중 하나. support catalog를 쓰면 `actions.json`에도 같은 id가 있어야 한다.
...
- 판정 지원 아이템은 `action` + `bonus` + 선택적 `effect`를 쓴다. support catalog를 아직 만들지 않을 때는 기본 id(`dc_down`, `extra_heart_damage`, `prevent_heart_loss`, `escape_boost`)만 쓰고, 시나리오 전용 effect id가 필요하면 검사 전에 `effects.json`을 먼저 만든다.
...
이 시점에 모든 NPC character의 `equipment`가 `{}`로 정규화된다. 서버 그래프는 `equips` edge의 owner를 player로만 허용하므로, NPC의 무기·갑옷·장신구는 `inventory` 소유만 유지한다. 플레이어 장비는 `player.json.equipment`에서만 다룬다.

게이트: 이 단계 뒤에 `characters/*.json`의 `equipment`에는 어떤 item id도 남아 있으면 안 된다. 장비가 필요한 NPC도 `inventory`만 사용한다. 이 규칙을 어기면 게임 시작 시 서버 그래프 검증에서 `equips owner must be player`로 실패하고, 클라이언트에는 `Failed to fetch`처럼 보일 수 있다.
...
`catalog-fill`은 `actions.json`, `effects.json`, `mbti.json`, `slots.json`을 `agency/story/catalogs/`에서 복사한다. 이미 있는 시나리오 전용 record는 보존하고, 고정 id는 템플릿 값으로 맞춘다.

- `actions.json`: 기본 전투 행동 id(`precise`, `reckless`, `defend`, `guarded`, `create_distance`, `talk`) 고정 catalog.
- `effects.json`: 기본 효과 id(`dc_down`, `escape_boost`, `prevent_heart_loss`, `heal`, `mp_restore`) 고정 catalog. 시나리오 전용 effect id가 필요하면 같은 파일에 추가 record로 둔다.
...
```bash
PYTHONPATH=server .venv/bin/python - <<'PY'
import asyncio
import tempfile
...
async def main():
    bundle = await init_graph_game(
        ...
        LocalFsGraphRepo(tempfile.mkdtemp(prefix="story_smoke_")),
        LocalFsScenarioRepo("scenarios"),
        locale="ko",
    )
    print("OK", bundle.progress.game_id, bundle.progress.active_subject_id)

asyncio.run(main())
PY
```

Windows PowerShell에서 heredoc이 안 맞으면 같은 Python 본문을 `@' ... '@ | .\.venv\Scripts\python.exe -` 형태로 실행한다.
...
- 인간형 race는 갑옷성 아이템을 `inventory`에 1개 이상 가져야 하고, 적이면 무기성 아이템도 `inventory`에 가져야 한다.

### skill
- 필수: `id`, `name`, `description`, `level`, `action`
- `action`은 기본 전투 행동 id(`precise`, `reckless`, `defend`, `guarded`, `create_distance`, `talk`) 또는 `actions.json`의 id를 가리킨다.
...
- `action`이 있으면 `precise`, `reckless`, `defend`, `guarded`, `create_distance`, `talk` 중 하나여야 한다.
