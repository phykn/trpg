...
각 race를 `scenarios/<name>/races/<id>.json`로 쓴다. `racial_skills`는 decompose 명단의 ID로 채운다 (스킬 파일은 아직 없지만 `--decomp`가 인정).
...
`racial_skills` (race 상속), `learned_skills` (decomp), `inventory` (decomp items 중 owner_character 매칭)을 채운다. `equipment` 슬롯은 비운 상태로 둔다 (Step 8에서 채움).
...
이 시점에 `characters/`가 디스크에 있다. 각 skill의 owner(race 또는 character)를 race의 `racial_skills` / character의 `learned_skills`에서 역추적해서, owner의 직업·레벨·역할을 보고 이름·설명·special_effect를 자연스럽게 맞춘다.
...
이 시점에 모든 character의 `equipment` 슬롯이 inventory의 `slot` 보고 자동 배치된다 (검 → weapon, 갑옷 → armor, 장신구 → accessory).
...
### location
- 필수: `id`, `name`, `description`, `connections`, `items`, `props`
...
- `action`은 `actions.json`의 id를 가리킨다.
- 보정이 있으면 `bonus`, 공통 효과가 있으면 `effect`에 `effects.json`의 id를 둔다.
