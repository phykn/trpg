...
- `action`은 `"attack"`, `"defend"`, `"flee"`, `"social"` 중 하나. support catalog를 쓰면 `actions.json`에도 같은 id가 있어야 한다.
...
- `actions.json`: 기본 행동 id(`attack`, `defend`, `flee`, `social`) 고정 catalog.
...
- `action`은 기본 행동 id(`attack`, `defend`, `flee`, `social`) 또는 `actions.json`의 id를 가리킨다.
...
- `action`이 있으면 `attack`, `defend`, `flee`, `social` 중 하나여야 한다.
