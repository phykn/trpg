...
6. `components/<name>/` — domain view files only. Currently: `Log`, `LogItem`, `RollResult`, `Composer`, `SendButton`, `StopButton`, `GameOverPanel`, `LevelUpPrompt`, `CombatStrip`, `ContextCard`, `IconButton`, `PanelBody`. Domains: `composer`, `log`, `combat`, `info-panel`. Import via `logic/<name>/` barrel — not directly. Screen-specific helpers (e.g. `screens/new-game/{Input,Section,SelectCard}.tsx`) live next to the screen, not here.
